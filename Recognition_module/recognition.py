...
face_to_compare = 'images/comparaison/iheb imen.jpg'
...
def comparefaces(img_path_to_comp="images/"):
    ...
    for facei in obj1:
        ...
        for hit in responseDlib['hits']['hits']:
            ...
            matches.append((hit['_source']['fname'], 1-float(hit['_score']), 'dlib'))

        for hit in responseFacenet['hits']['hits']:
            # double score=float(hit['_score'])
            # if (float(hit['_score']) > 0.93):
            matches.append((hit['_source']['fname'], float(hit['_score'])-1, 'facenet'))

        for hit in responseArcface['hits']['hits']:
            # double score=float(hit['_score'])
            # if (float(hit['_score']) > 0.93):
            matches.append((hit['_source']['fname'], float(hit['_score'])-1, 'arcface'))
